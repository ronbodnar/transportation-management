...
function updateChartColors() {
  var charts = [
    ...
    shipmentsChart,
    facilityChart,
    coiShipmentTargetChart,
    ontarioShipmentTargetChart,
    riversideShipmentTargetChart,
    comparisonChart,
  ...
  charts.forEach((chart) => {
    ...
    if (
      chart == coiShipmentTargetChart ||
      chart == ontarioShipmentTargetChart ||
      chart == riversideShipmentTargetChart
    ) {
...
if (document.getElementById("facilityChart")) {
  ...
  facilityChart = new Chart(ctx, {
    type: "pie",
    data: {
      labels: ["Coldstore COI", "Coldstore Ontario", "Coldstore Riverside"],
      datasets: [
        {
...
if (document.getElementById("timeUtilizationChart")) {
  ...
  timeUtilizationChart = new Chart(ctx, {
    ...
    data: {
      ...
      //labels: ["Driving", "Backhauls", "Waiting @ Warehouse", "Waiting @ Plant", "Yard Moves", "Breaks", "Fueling", "Shop", "Breakdowns", "Broken Trailers"],
      datasets: [
        {
...
function setData(facility, data) {
  ...
  var result = []
  if (facility === "coi") {
    target = data["targets"]["coi"]
  } else if (facility === "ontario") {
    target = data["targets"]["ontario"]
  } else if (facility === "riverside") {
    target = data["targets"]["riverside"]
  } else {
    console.log('Unhandled facility: "' + facility + '"')
  }

  var sent = facility === "coi" ? 26 : facility === "ontario" ? 22 : 4

  result = [...new Array(8)].map((i, idx) => target)

  let deduction1 = randomIntFromInterval(
    facility === "riverside" ? 0 : 2,
    target / 5
  )
  let deduction2 =
    deduction1 +
    randomIntFromInterval(facility === "riverside" ? 0 : 2, target / 5)
  let deduction3 =
    deduction2 +
    randomIntFromInterval(facility === "riverside" ? 0 : 2, target / 5)
  let deduction4 =
    deduction3 +
    randomIntFromInterval(facility === "riverside" ? 1 : 2, target / 5)
  let deduction5 =
    deduction4 +
    randomIntFromInterval(facility === "riverside" ? 1 : 2, target / 5)
  let deduction6 =
...
var coiShipmentTargetChart = null
var ontarioShipmentTargetChart = null
var riversideShipmentTargetChart = null
if (
  document.getElementById("coiShipmentTargetChart") &&
  document.getElementById("ontarioShipmentTargetChart") &&
  document.getElementById("riversideShipmentTargetChart")
) {
  const labels = [
    "12 AM",
    "3 AM",
  ...
  const ctxIndustry = document
    .getElementById("coiShipmentTargetChart")
    .getContext("2d")
  const ctxOntario = document
    .getElementById("ontarioShipmentTargetChart")
    .getContext("2d")
  const ctxriverside = document
    .getElementById("riversideShipmentTargetChart")
    .getContext("2d")
  coiShipmentTargetChart = new Chart(ctxIndustry, {
    type: "scatter",
    data: coiData,
    options: chartOptions,
  })
  ontarioShipmentTargetChart = new Chart(ctxOntario, {
    type: "scatter",
    data: ontarioData,
    options: chartOptions,
  })
  riversideShipmentTargetChart = new Chart(ctxriverside, {
    type: "scatter",
    data: riversideData,
    options: chartOptions,
  })
...
if (document.getElementById("activeDriverChartMin")) {
  ...
  activeDriverChartMin = new Chart(ctx, {
    type: "bar",
    data: {
      labels: ["Plant", "COI", "Ontario", "Riverside"],
      datasets: [
        {
...
if (document.getElementById("activeDriverChart")) {
  ...
  activeDriverChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Drivers at Home Terminal",
          backgroundColor: [
            "#3D68BB",
            "#E99A3C",
            "#218B94",
