...
if (strcmp($requestMethod, 'GET') === 0) {
    ...
    if (strcmp($action, 'northList') === 0) {
        $doors = $database->dockDoorRepository->getDoorData('NORTH');
        ...
        foreach ($doors as $key => $value) {
            ...
            if (strpos((is_null($value['status']) ? 1 : $value['status']), "EMPTY") !== false || ($shipment == null && $value['trailerId'] != 0)) {
        ...
        $doors = $database->dockDoorRepository->getDoorData('SOUTH');
        ...
        $availableDoors = $database->dockDoorRepository->getAvailableDoors();
        ...
        echo $database->dockDoorRepository->countOpenDoors();
    } else {
    ...
    if (strcmp($action, 'add') === 0) {
        if (isset($_POST['door']) && isset($_POST['driverId'])) {
            $database->dockDoorRepository->addToDoor($_POST['door'], $_POST['driverId']);
        } else {
        ...
        if (isset($_POST['door'])) {
            $database->dockDoorRepository->removeFromDoor($_POST['door']);
        } else {
